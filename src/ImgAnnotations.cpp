...
void IA::ImgAnnotations::saveToFile(const string &filePath)
{
	// try to open the file
 	...
 	string file2name = filePath+".csamples";
 	std::ofstream out2(file2name, std::ios::trunc);
 	...
 	if (!out2) {
		cerr << "IA::ImgAnnotations::saveToFile()" << endl;
		cerr << "\tError opening the file: " << file2name << endl;
		return;
	}
 	// save annotations file path
 	_annotationsFilePath = filePath;
	...
	BOOST_FOREACH (File *iFile, getFiles()) {
		...
		out2 << iFile->getFilePath() << " ";
		out2 << iFile->numOfObjects() << " ";
		...
		BOOST_FOREACH (Object *iObj, iFile->getObjects()) {
			...
			BOOST_FOREACH (const string& iProperty, iObj->getProperties()){
				...
				std::istringstream ss(iObj->get(iProperty));
				while( ss.good() )
				{
				    string substr;
				...
				//out2 << result[0]<<" " << result[1]<<" "<< result[2]<<" "<< result[3]<<" "<< " ";
			}
